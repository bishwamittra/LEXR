...
class DagSATEncoding:
    ...
    def __init__(self, D, testTraces, optimization=False):
...
#        self.visitedPositions = set()
    ...
    def encodeFormula(self, unsatCore=True):
        ...
        if(self.optimization == False):
            ...
            self.solver.assert_and_track(And([self.y[(self.formulaDepth - 1, traceIdx, 0)] for traceIdx in range(
                len(self.traces.acceptedTraces))]), 'accepted traces should be accepting')
            ...
            """  
            Declare unit soft clauses

            """
            # print("\nCalling the optimization solver\n")

            for traceIdx in range(len(self.traces.acceptedTraces)):
                self.solver.add_soft(self.y[(self.formulaDepth - 1, traceIdx, 0)])
            for traceIdx in range(len(self.traces.acceptedTraces), len(self.traces.acceptedTraces+self.traces.rejectedTraces)):
                self.solver.add_soft(Not(self.y[(self.formulaDepth - 1, traceIdx, 0)]))
            ...
            for i in range(1, self.formulaDepth):
                                                         ...
                                                         And([Implies(
                                                             ...
                                                             And(
                                                                 [self.y[(i, traceIdx, timestep)]
                                                                  ...
                                                                  Implies(
                                                                     self.y[(
                                                                         leftArg, traceIdx, timestep)],
                                                                     self.y[(
                                                                         rightArg, traceIdx, timestep)]
                                                                 )
                                                                     for timestep in range(tr.lengthOfTrace)]
                                                             )
                                                         )
                ...
                if '!' in self.listOfOperators:
                    ...
                    self.solver.assert_and_track(Implies(self.x[(i, '!')],
                                                         And([
                                                             Implies(
                                                                 self.l[(
                                                                     i, onlyArg)],
                                                                 And([
                                                                     self.y[(i, traceIdx, timestep)] == Not(
                                                                         self.y[(onlyArg, traceIdx, timestep)])
                                                                     for timestep in range(tr.lengthOfTrace)
                                                                 ])
                                                             )
                                                 ...
                                                 'semantics of negation for trace %d and depth %d' % (
                                                     traceIdx, i)
                                                 )
                    ...
                    self.solver.assert_and_track(Implies(self.x[(i, 'G')],
                                                         And([
                                                             Implies(
                                                                 ...
                                                                 And([
                                                                     self.y[(i, traceIdx, timestep)] ==
                                                                     And([self.y[(onlyArg, traceIdx, futureTimestep)] for futureTimestep in tr.futurePos(
                                                                         timestep)])
                                                                     for timestep in range(tr.lengthOfTrace)
                                                                 ])
                                                             )
                                                 ...
                                                 'semantics of globally operator for trace %d and depth %d' % (
                                                     traceIdx, i)
                                                 )
                    ...
                    self.solver.assert_and_track(Implies(self.x[(i, 'F')],
                                                         And([
                                                             Implies(
                                                                 ...
                                                                 And([
                                                                     self.y[(i, traceIdx, timestep)] ==
                                                                     Or([self.y[(onlyArg, traceIdx, futureTimestep)] for futureTimestep in tr.futurePos(
                                                                         timestep)])
                                                                     for timestep in range(tr.lengthOfTrace)
                                                                 ])
                                                             )
                                                 ...
                                                 'semantics of finally operator for trace %d and depth %d' % (
                                                     traceIdx, i)
                                                 )
                ...
                if 'X' in self.listOfOperators:
                    ...
                    self.solver.assert_and_track(Implies(self.x[(i, 'X')],
                                                             ...
                                                             Implies(
                                                                 ...
                                                                 And(
                                                                     And([
                                                                         self.y[(i, traceIdx, timestep)] ==
                                                                         self.y[(
                                                                             onlyArg, traceIdx, tr.nextPos(timestep))]
                                                 ...
                                                 'semantics of neXt operator for trace %d and depth %d' % (
                                                     traceIdx, i)
                                                 )
                                                             ...
                                                             And([
                                                                 ...
                                                                 Or([
                                                                     And(
                                                                         ...
                                                                         [self.y[(rightArg, traceIdx, tr.futurePos(
                                                                             timestep)[qIndex])]]
                                                                     )
