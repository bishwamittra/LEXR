...
class Translator:
    ...
    def formula_parser(self):
        if self.formulaType in {1, 2, 3}:
            ...
            raise ValueError(
                'Ooops! You typed a formula with mixed past/future operators')
    ...
    def translate(self):
        self.translated_formula = translate_bis(
            self.parsed_formula, self.formulaType, var='v_0')+";\n"
    ...
    def createMonafile(self, flag, path):
        program = self.buildMonaProgram(flag)
        try:
            # print(path+'.mona')
            with open(path+'.mona', 'w+') as file:
                file.write(program)
                file.close()
        except IOError:
            print('Problem with the opening of the file!')

    def invoke_mona(self, path):
        ...
        # else:
        try:
            subprocess.call('mona -w -n -q '+path+'.mona > ' +
                            path + '.log', shell=True)
            subprocess.call(
                'mona -u -gw '+path+'.mona > ' + path + '_inter.dot', shell=True)
            # print('mona -w -n -q '+path+'.mona > ' +
            #                 path + '.log')
            # print('mona -u -gw '+path+'.mona > ' + path + '_inter.dot')
        except subprocess.CalledProcessError as e:
            print(e)
            exit()
        except OSError as e:
